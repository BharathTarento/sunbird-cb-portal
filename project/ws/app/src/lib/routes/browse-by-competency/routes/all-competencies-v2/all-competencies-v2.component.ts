...
@Component({
  selector: 'ws-app-all-competencies-v2',
  templateUrl: './all-competencies-v2.component.html',
  styleUrls: ['./all-competencies-v2.component.scss'],
})
export class AllCompetenciesV2Component implements OnInit, OnDestroy {
  titles = [
  ...
  constructor(
    ...
    private competencySvc: BrowseCompetencyService,
  ...
  getAllCompetencies() {
    ...
    const request = { 'search': { 'type': 'Competency Area' }, 'filter': { 'isDetail': true } }
  ...
  async callCompetencySearch(request: any) {
    return new Promise<any>((resolve, reject) => {
      if (request && request) {
        this.competencySvc.searchV6(request).subscribe(results => {
            resolve({ results })
          },                                           (error: any) => {
            reject(error)
          },
